...
export function parseHighwayHustleHostPayload(data: MiniGamePayloadType): HighwayHustleData {
  ...
  for (let i = 0; i < payload.entitiesLength(); i++) {
    players.push({
      id: decodeURI(payload.entities(i).id()),
...
export function parseHighwayHustlePlayerPayload(
  data: MiniGamePayloadType
): HighwayHustlePlayerData {
  const payload: HighwayHustlePlayerPayload = data.gamestatepayload(
    new HighwayHustlePlayerPayload()
  )
...
export function parseHighwayHustleResultPayload(data: MiniGamePayloadType): HighwayHustleResult {
  const payload: HighwayHustleResultPayload = data.gamestatepayload(
    new HighwayHustleResultPayload()
  )
