  ...
  <div class="w-full h-full flex">
    <div class="grid grid-rows-4 text-center my-auto max-w-full h-full gap-y-12 max-h-[75%] w-full">
      ...
      <div class="grid grid-cols-2 row-span-2 w-full h-full rounded-3xl overflow-hidden">
        <button class="active:bg-opacity-100 bg-opacity-50 bg-red-500 border-r-2 border-dashed" @click="decrement"
          :disabled="data.amount <= 0">
          <span class="text-8xl font-bold">-</span>
        </button>
        <button class="active:bg-opacity-100 bg-opacity-50 bg-green-500 border-l-2 border-dashed" @click="increment">
          <span class="text-8xl font-bold">+</span>
        </button>
      </div>
      <button class="bg-red-400 active:bg-red-500 rounded-full text-white grid p-4" @click="submitAnswer">
