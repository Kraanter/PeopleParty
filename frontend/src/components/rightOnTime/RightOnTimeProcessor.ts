...
import type {
  RightOnTimeData,
  RightOnTimeResultPair,
  RightOnTimeResults,
  RightOnTimeRoundResultPair,
  RightOnTimeRoundResults
...
export function parseRightOnTimeRoundResults(
  data: MiniGamePayloadType,
  clientName: string = ''
): RightOnTimeRoundResults {
  const miniGameResultPayload: RightOnTimeRoundResultPayload = data.gamestatepayload(
    new RightOnTimeRoundResultPayload()
  )
...
export function parseRightOnTimeResults(data: MiniGamePayloadType): RightOnTimeResults {
  const miniGameResultPayload: RightOnTimeResultPayload = data.gamestatepayload(
    new RightOnTimeResultPayload()
  )
