...
<template>
  ...
  <template v-else-if="viewState === ViewState.MiniGame && playerData.is_eliminated">
    <div class="flex flex-col h-full justify-center items-center gap-4 p-6 text-center">
      ...
      <p class="text-xl text-gray-300">
        You survived {{ playerData.current_round - 1 }} round{{
          playerData.current_round !== 2 ? 's' : ''
        }}.
      </p>
  ...
  <template
    v-else-if="viewState === ViewState.MiniGame && playerData.phase === ShellShufflePhase.REVEAL"
  >
  ...
  <template
    v-else-if="viewState === ViewState.MiniGame && playerData.phase === ShellShufflePhase.SHUFFLE"
  >
    <div class="flex flex-col h-full justify-center items-center gap-6 p-6 text-center">
  ...
  <!-- In-game: GUESS phase -->
  <template
    v-else-if="viewState === ViewState.MiniGame && playerData.phase === ShellShufflePhase.GUESS"
  >
  ...
  <template
    v-else-if="
      viewState === ViewState.MiniGame && playerData.phase === ShellShufflePhase.ROUND_RESULT
    "
  >
    <div class="flex flex-col h-full justify-center items-center gap-6 p-6 text-center">
  ...
  <template v-else-if="viewState === ViewState.Results">
    <div class="flex flex-col h-full justify-center items-center gap-4 p-6 text-center">
      <template v-if="personalResult">
        ...
        <p class="text-8xl text-yellow-300 font-bold">
          {{ formatOrdinals(personalResult.placement) }}
        </p>
        <p class="text-xl text-gray-300">
          Survived {{ personalResult.rounds_survived }} round{{
            personalResult.rounds_survived !== 1 ? 's' : ''
          }}
        </p>
      </template>
      <template v-else>
