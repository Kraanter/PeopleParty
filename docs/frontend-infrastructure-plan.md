...
| 2 | Migrate `shellShuffle` (reference), then `marbleMania` as go/no-go gate (part of Recommendation B) | Done | Implemented 2026-08-26. **Go/no-go verdict: GO** — the pooled-entity design earns its keep; no reason found to revise the composables. **Prerequisite fix, found during design validation before any migration code was written**: `useGameCanvas.ts`'s `worldSize` letterbox mode had a real, already-shipped bug — `applySize()` set `root.position.set(offsetX, offsetY)` using offsets computed against the *full container* size, but the canvas element itself is only resized to the *fitted (smaller)* size, so content was shifted by that offset inside a canvas that was never that much bigger, clipping it off the far edge whenever the container's aspect ratio didn't exactly match `worldSize`'s. Fixed to `root.position.set(0, 0)` — the canvas element is already exactly the fitted rectangle; centering it within its container is a CSS/flex concern, not `root`'s. Re-verified the existing `/dev/pixi-playground` checks afterward (all still pass) plus explicitly re-tested under a non-4:3 container via the new shellShuffle harness (see below) to confirm the fix holds. **shellShuffle**: single shared `Graphics`, redraw-everything-on-invalidate — deliberately does *not* use `createEntityLayer` (pooling is marbleMania's job as the explicit gate; shellShuffle's job is proving sizing/letterbox/lifecycle). New `frontend/src/components/shellShuffle/ShellShuffleGameView.vue` owns `useGameCanvas`+`useSnapshotBuffer`; `HostView.vue` slimmed to routing, forwarding pushes via a template ref. **marbleMania**: reframed its fixed camera viewport (not the scrolling world) as `useGameCanvas`'s `worldSize: {800,600}` — this let the composable's own `ResizeObserver`+letterbox fully replace the old `window.innerHeight`-based sizing and, as a side effect, fixed a latent bug where the canvas rendered perfectly square below 600px of available height. Camera-follow was upgraded to real per-frame `dt` interpolation via `startAnimating` (**user's explicit choice**, made after being shown the real cost: this switches the gameplay canvas from network-cadence-only redraws to continuous rendering for the whole simulation phase). Three `createEntityLayer` instances replace the single shared `Graphics` + the `:key`-driven Application recreation: `obstacleLayer` (always-synced, visibility-toggled, shape drawn once per display via a flag since obstacles never change) → `marbleLayer` (always-synced, body redrawn every update — cheap at ≤16 marbles) → `offScreenIndicatorLayer` (synced only with the currently-off-screen subset, on top) — in that fixed z-order, preserving today's real draw-order guarantee. Two new child components (`MarbleManiaGameView.vue`, `MarbleManiaResultsView.vue`), since it has two separate canvases; `HostView.vue` slimmed to routing plus a `getPlayerColor` map shared across both children (needed so a player's marble color survives the gameplay→results transition, which a fresh per-child map wouldn't). **Real bug caught by live verification** (not by two independent design-validation passes beforehand, both of which missed it): `animateCamera`'s per-frame step preserved the *previous* `cameraOffset.x` instead of re-deriving it from `world_min.x` every call like the original `updateCamera()` did ("always show full width") — harmless if a placement-phase snapshot always arrives before the first simulation-phase one (true in real gameplay), but if `push()`'s first-ever call is already phase 1/2, `cameraOffset.x` silently stuck at its `0` default, shifting every marble off-camera. Confirmed live via the marbleMania harness driving "start simulation" without a preceding "placement" push — most marbles rendered as invisible (culled) or badly mispositioned; fixed by re-deriving `x` from `buffer.current.world_min.x` on every `animateCamera` tick. Also fixed as a natural side effect of writing correct, explicit v8 fluent calls (not a deliberate fix): the world-boundary lines never actually rendered white in the original code — v8's `GraphicsContext` only resolves a shape's fill/stroke on the *next* `fill()`/`stroke()` call, so the boundaries' own unclosed `lineStyle`/`moveTo`/`lineTo` sat unresolved until whichever unrelated shape's `endFill()` came next, rendering in that shape's color instead (green during placement, red during simulation). `MarbleManiaSpriteMap.ts` deleted (confirmed dead — marbles/obstacles are vector-drawn, never sprited). **Verification**: two new dev-only harness routes (`/dev/shellshuffle-harness`, `/dev/marblemania-harness`, same `import.meta.env.DEV` convention as the Phase 1 playground) mount the real migrated components directly and drive them via a dev-only `__devPush` hook with hand-written plain-object scenarios — real FlatBuffers were deliberately not used, since the decode layer (`*Processor.ts`) isn't touched by this migration and round-tripping through it would add real effort for no extra coverage. Drove both live via the same scratch Playwright/local-Chrome setup from Phase 1: confirmed canvas-element-count stays at exactly 1 across every scenario including six sequential marble-finish events (the actual go/no-go check), zero console/page errors, and visually confirmed correct rendering (colors, depth-scaled cups, lifted-cup ball reveal, letterboxing under a deliberately non-4:3 container, culled/off-screen indicators, results screens) via screenshots. `type-check`/`lint:check`/`format:check` all clean. Nothing committed to git. **Anyone touching camera/position state in either migrated view again: re-drive the relevant harness starting from each phase independently (not just phase 0 → 1 in sequence) — the `cameraOffset.x` bug above only reproduced when a later phase's push arrived without an earlier one first.** **Addendum, 2026-09-15 — real user-reported regression found by actually running the game, not the harness**: running `marbleMania` live (not the dev harness) showed obstacles never rendering, the camera locking onto one player, and different (worse) camera smoothing than pre-migration. Root cause of all three, confirmed via three independent investigations (git archaeology, a fresh backend read, a fresh frontend audit) plus a Plan-agent validation pass on the fixes: (1) `HostView.vue`'s `update()` set `viewState` and called `gameViewRef.value?.push(...)` in the same synchronous tick — the child's template ref isn't populated yet (Vue defers it), silently dropping that call. Harmless for ordinary marble-position updates (the next one supersedes it 50ms later) except the backend sends obstacle entities *exactly once ever* per game (`sended_obstacles`, only reset by Pause→Resume) — the dropped message was the only one that would ever carry them. A second bug compounded it: the merge logic only patched entities already known by id, never adding ones it hadn't seen, so even a resend could never backfill obstacles once the buffer had been populated without them. **Fix**: replaced the ref-push with a `shallowRef` prop (`:host-data`) the child `watch()`es with `{ immediate: true }` — eliminates the race structurally instead of timing around it, matching the pattern `MarbleManiaResultsView.vue` already used correctly for its own one-shot data — plus a plain union-by-id merge (the backend sends full entities every tick, never a partial diff, so a selective field-merge was unnecessary complexity, not a safety net). (2) `computeTargetCameraTop()`'s `marbles.reduce(...)` had no seed, so it started from `marbles[0]` even if already finished; a finished marble's position is frozen at the finish line (the map's max-y point), so once whichever marble happened to be at array index 0 finished while leading, no active marble's position could ever exceed it and the camera froze on it permanently — confirmed pre-existing (introduced August 2025, well before the migration) and ported faithfully rather than introduced. **Fix**: filter finished marbles out *before* reducing, not just inside the comparator; validation caught that the fallback (all marbles finished) must drop the finished-guard entirely rather than re-adding it, since that fallback is the sole driver of camera behavior for the final ~2 seconds of every race, not a rare edge case. (3) The original 25%-per-call camera smoothing ran once per incoming network payload (~50ms, backend `target_fps=20`); the migrated version reused the same `0.25` constant inside a per-rendered-frame formula, running ~3x more often than the target itself changes — converging almost entirely within 1-2 frames then sitting idle, a "snap then pause" feel instead of the original's continuous trail. **Fix**: recalibrated the ease-factor's time reference from the frame period (`1000/60`) to the actual network tick (`NETWORK_TICK_MS = 50`), preserving genuinely smooth per-frame interpolation while matching the originally-tuned convergence rate. **Verification gap identified and closed**: the existing `/dev/marblemania-harness` bypassed `HostView.vue` entirely (drove the child's dev-only push hook directly) and always included obstacles in every synthetic message — structurally unable to catch either the ref-race or the one-shot-obstacle interaction, and never checked convergence *rate* for the smoothing bug. Rebuilt the harness to mount `HostView` itself and drive it via a new `__devUpdateHost` hook that exercises the exact same synchronous `viewState`/`hostData` assignment `update()` uses, with scenarios modeling the backend's real behavior: obstacles in message 1 only, a marble at index 0 finishing while clearly leading, an all-finished `game_phase: 2` tail, and a simulated Pause→Resume resend. Added a dev-only `__devCameraY` getter and sampled it every ~16ms via Playwright to check convergence *quantitatively*, not just visually. All three fixes reverified live: obstacles visible after the first message and still present through obstacle-less subsequent messages and a simulated resume; `smoothedCameraY` moved +240 units in the 800ms after the leader finished (not frozen); samples during simulation showed gradual, monotonic convergence (deltas ~5-10 units/16ms) rather than one large jump followed by zero deltas; canvas-element-count stayed at exactly 1 across every scenario; zero console/page errors. `type-check`/`lint:check`/`format:check` all clean. Nothing committed to git. **The real lesson, worth restating for future phases**: a dev-only harness that bypasses the real parent-to-child wiring to make testing more convenient can hide exactly the class of bug (mount-timing races interacting with one-shot backend data) that only shows up running the actual game — drive harnesses through the real component entry point (`update()`/props), not a shortcut into the middle of it, whenever the thing being tested is the wiring itself.** |
