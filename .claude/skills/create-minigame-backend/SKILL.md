...
Implement the full C++ backend for a new PeopleParty minigame — lifecycle management, game
states, physics, timers, FlatBuffer serialization, and wiring into the game loop.
...
**Required reading — base classes** (read first, in full):
- `backend/src/minigames/minigame.h` and `minigame.cpp` — the `MiniGame` class
- `backend/src/game_state.h` — the `GameState` base class that `MiniGame` extends

Read these two together and note the split carefully — it's easy to misattribute:
- `GameState` declares the pure virtuals `process_input(const Message*, Client*)`,
  `update(int delta_time)`, and `clients_changed(int client_id, bool joined)`, plus a protected
  pure virtual `finished()` and the public `int update_interval` member.
- `MiniGame` extends `GameState` and adds its own pure virtuals: `start_introduction()`,
  `start_minigame()`, `start_result()`, `pause()`, `resume()`, `getMinigameResult()`,
  `get_display_name()`, `get_camel_case_name()`, `get_description()`, plus a protected pure
  virtual `process_input(const MiniGamePayloadType*, Client*)` (a *different* overload from
  `GameState`'s `Message*` one — `MiniGame` already implements the `Message*` overload in
  `minigame.cpp`, dispatching into the `MiniGamePayloadType*` one you write).
- `MiniGame` also implements `finished()` for you (in `minigame.cpp`) — it records the leaderboard
  placement and advances `Game` to the next state. **Your minigame calls `finished()`, it does not
  override it.**
- `MiniGame` provides a non-virtual `start()` that just calls `start_introduction()` — call
  `start()` from wherever the game loop spins up a new minigame (already handled by `Game`).
- **`update(int)` and `clients_changed(int, bool)` are `GameState` responsibilities, not
  `MiniGame` ones** — every concrete minigame must implement both even though they aren't declared
  in `minigame.h`.
- `introduction_update(int dt)` is **not part of either base class** — it's a convention every
  existing minigame reinvents as a private helper called from its own introduction timer. Feel
  free to name it something else, but following the convention keeps the codebase consistent.
- Defaults from `MiniGame`: `min_players = 1`, `max_players = -1` (no limit),
  `introduction_time = 7 SECONDS`, plus a protected `ThreadTimer timer` member you can reuse instead
  of declaring your own if one timer is enough.

**Required reading — reference games** (read ALL files in each directory):
- `backend/src/minigames/teambased_pong/` — multi-phase rounds (ROUND_PREP → PLAYING →
  ROUND_RESULT), Box2D physics, team management, elimination, averaged joystick input. Note: this
  game's own files use `_minigame.h/.cpp` (no second underscore) and class suffix `_MiniGame` —
  check the exact filename before assuming.
- `backend/src/minigames/shell_shuffle/` — newest addition; phase-enum pattern, Allman-style
  formatting throughout, clean reference for a simpler phase-based game
- `backend/src/minigames/highway_hustle/` — entity-based with map class, moving entities, joystick
  input, custom AABB collision, obstacle spawning, lane system
- `backend/src/minigames/marble_mania/` — Box2D physics with gravity, procedural map generation,
  placement + simulation phases, PHYSICS_SCALE coordinate scaling. Its map class
  (`marble_mania_map.h/.cpp`) is the one exception to the "raw pointers only" rule below — it uses
  `std::unique_ptr`/`make_unique` internally. Don't take that as license to use smart pointers
  elsewhere; every other minigame in the codebase uses raw pointers.
...
- `backend/src/minigames/memory_mixer/` — grid-based, elimination, round progression, `build_grid()`
  reusable builder pattern
- `backend/src/minigames/crazy_counting/` — simpler entity pattern with steering behaviors,
  `clients_changed` override
- `backend/src/minigames/right_on_time/` — multi-round timing game with per-round targets (class
  is `RightOnTime_Minigame`, note the lowercase `g`)
- `backend/src/minigames/rps_bracket/` — bracket/tournament, match management with stack-based
  results
- `backend/src/minigames/launch_party/` — reaction time, practice rounds, phase-based with lights,
  `std::chrono` timing (class is `LaunchParty_Minigame`, note the lowercase `g`)
- `backend/src/minigames/business_bailout/` — stock-market style, bail mechanic, fine-grained
  100ms updates (class is `BusinessBailout_Minigame`, note the lowercase `g`)

**Required reading — infrastructure**:
- `backend/src/game.h` and `game.cpp` — `Game::add_minigames()` queue, `update_leaderboard()`,
  `nextGameState()`, `handle_new_minigame()`
- `backend/src/party.h` — `send_gamestate()`, `send_message()` signatures and filter lambda
  patterns
- `backend/src/util/thread_timer.h` — `setInterval()`, `setTimeout()`, `clear()`, `pause()`,
  `resume()`
...
- `backend/src/util/flatbuffer/flatbuffer_message_builder.h` and `.cpp` — only used for handshake
  (Host/Join) messages, NOT minigame payloads
...
2. Schemas have been built (`npm run build:macos` or platform variant) so that
   `backend/src/flatbuffer/messageClass_generated.h` contains the generated types
3. Read `messageClass_generated.h` to find the exact `GameStateType_` and `GameStatePayload_`
   enum values for the new game's payloads
...
**Naming — there is no single enforced rule, existing code is split**:
- File suffix: `_mini_game.h/.cpp` (highway_hustle, marble_mania, crazy_counting, memory_mixer,
  launch_party, right_on_time, shell_shuffle, unscrambled) vs `_minigame.h/.cpp` (business_bailout,
  rps_bracket, teambased_pong). Default to `_mini_game.h/.cpp` for new games — it's the majority.
- Class suffix: `_MiniGame` (most games) vs `_Minigame` (`BusinessBailout_Minigame`,
  `LaunchParty_Minigame`, `RightOnTime_Minigame`). Default to `_MiniGame` — it's the majority.
...
public:
    ...
    // Lifecycle — pure virtuals declared on MiniGame
    ...
    // Pure virtuals declared on GameState (MiniGame's base) — every minigame needs these too
    void update(int delta_time) override;
    void clients_changed(int client_id, bool joined) override;

    // Metadata — pure virtuals declared on MiniGame
    std::string get_display_name() override;
    std::string get_camel_case_name() override;
    std::string get_description() override;

protected:
    // Pure virtual declared on MiniGame — dispatches minigame-specific payloads
    void process_input(const MiniGamePayloadType* payload, Client* from) override;

private:
    // Timers — ALL must be .clear()-ed in destructor. `timer` is inherited from MiniGame;
    // declare extra ones here only if you need more than one.
...
#### introduction_update(dt) — your own private convention, not an override
```cpp
...
#### update(dt) — implements the GameState pure virtual, main game loop
```cpp
...
void {GameName}_MiniGame::start_result()
{
    ...
    {
        finished();  // implemented by MiniGame — updates leaderboard, advances Game state
    }, result_time);
}
```
...
#### clients_changed — implements the GameState pure virtual; empty unless mid-game join/leave handling needed
```cpp
...
For games with multiple phases (like teambased_pong or shell_shuffle), use a phase enum pattern:
...
**IMPORTANT**: `GameStateType_` and `GameStatePayload_` values are auto-generated by `builder.sh`
from the schema table names (stripping `"Payload"` suffix). Read `messageClass_generated.h` to
find the exact enum values for your game.
...
Only `marble_mania` and `teambased_pong` currently use Box2D — it's not a dependency every
minigame needs. If your game needs physics, follow those two patterns:
...
For custom collision handling, use a `b2ContactListener` subclass — see `PongContactListener` in
teambased_pong.

### Step 9 — Threading Rules

**CRITICAL**: Timer callbacks (`setInterval`, `setTimeout`) run on detached background threads,
NOT the uWS event loop.

- All WebSocket sends through `party->send_gamestate()` or `party->send_message()` are already
  safe — they use `server_loop->defer()` internally
...
The `camelCase` key must match `get_camel_case_name()` and must match the frontend component
folder name.
...
3. Verify `get_camel_case_name()` matches the frontend component folder and FlatBuffer schema
   naming
4. Verify all `GameStateType_` and `GameStatePayload_` enum references match
   `messageClass_generated.h`
5. Build to check for compile errors: `cd backend/build && cmake --build .`
6. Confirm `update(int)` and `clients_changed(int, bool)` are both implemented — they're easy to
   forget since they aren't declared in `minigame.h` itself

## Quick Reference — Code Style

| Element | Convention | Example |
|---------|-----------|---------|
| Indentation | 4 spaces (minigame code); some core files use 2 | — |
| Braces | Mixed — Allman common, K&R common for ctors/dtors | match the file |
| Methods | snake_case | `start_introduction()`, `send_host_update()` |
| Variables | snake_case | `remaining_time`, `delta_time` |
| Classes | PascalCase + `_MiniGame` (majority) or `_Minigame` (3 exceptions) | `TeambasedPong_MiniGame` |
| Header guards | `#ifndef PEOPLEPARTY_BACKEND_{NAME}_H` | Not `#pragma once` |
| Time | DSL macros | `7 SECONDS`, `500 MILLISECONDS` |
| Memory | Raw pointers, manual new/delete (one exception: marble_mania_map) | No smart pointers elsewhere |
...
- **Forgetting `update()`/`clients_changed()`** — they're inherited from `GameState`, not declared
  in `minigame.h`, so it's easy to overlook them when copying the "MiniGame virtuals" list
- **Wrong `GameStateType_` enum values** — always read `messageClass_generated.h` after schema
  build, never guess
- **Creating FlatBuffer strings after the table** — strings/vectors MUST be created before
  `Create{Table}()`
- **Sending from timer threads without defer** — causes uWS crashes; use `party->send_gamestate()`
  which handles this
- **Not skipping host in player population** — host is a Client too; always check
  `client->party->host == client`
- **`get_camel_case_name()` mismatch** — must exactly match frontend folder name and
  `add_minigames()` key
